...
if [ -z "${AGENTVER}" ] || [ "${AGENTVER}" == 'null' ]; then
    log "ERROR: Unable to determine the latest agent version (GitHub API unreachable or rate limited)"
    exit 1
fi
...
# Download the new binary first, while the existing agent keeps running, so
# a failed download leaves the running install untouched.
log "Downloading new tactical agent..."
if ! wget -q -O ${agentBin}.new "${agentDL}"; then
    log "ERROR: Unable to download tactical agent. Leaving the existing agent in place."
    rm -f ${agentBin}.new
    exit 1
fi
...
# Smoke test the new binary before committing to it.
log "Verifying downloaded binary..."
if ! ${agentBin}.new --version >/dev/null 2>&1; then
    log "ERROR: Downloaded binary failed to execute. Leaving the existing agent in place."
    rm -f ${agentBin}.new
    exit 1
fi
...
# Stop the service only now that we have a verified replacement ready.
...
if systemctl is-active --quiet ${agentSvcName}; then
    ...
    if [ -f "${agentBin}.bak" ]; then
        log "Restoring previous binary from backup..."
        mv ${agentBin}.bak ${agentBin}
        systemctl start ${agentSvcName} || true
    fi
fi

# Clean up this script
log "Update completed, cleaning up..."
rm -- "$0"
EOF
