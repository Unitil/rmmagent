...
hostCheckandAdd() {
    ...
    if ! grep -qF "${ip} ${host}" /etc/hosts; then
...
DownloadMeshAgent() {
  ...
  if ! ( wget $url/meshagents?id=$machineid -O ${meshTmpBin} || curl -L --output ${meshTmpBin} $url/meshagents?id=$machineid ); then
  ...
  if ! ( wget $url/meshsettings?id=$meshid -O ${meshTmpMsh} || curl -L --output ${meshTmpMsh} $url/meshsettings?id=$meshid ); then
...
# Handle uninstall before any other setup so removal needs no config,
...
if [ -z "${AGENTVER}" ] || [ "${AGENTVER}" == 'null' ]; then
    echo "ERROR: Unable to determine the latest agent version (GitHub API unreachable or rate limited)"
    exit 1
fi
